...
|---|---|---|
| `0x189A` X | RAM 64h = linearised MAP − 64 + correction | 43 61 80 99 118 136 155 219 |
| `0x18A3` Y | RAM 32h:33h = Timer2 period ≫ 4 | 142 153 166 181 200 222 250 285 307 333 363 400 454 526 588 666 |
| `0x1EF0` X | RAM 72h = ADC ch1 raw | 65 91 117 143 169 |
...
1. ADC sweep: ch3 → 71h → linearised via `0x1887/0x1889` → **38h (MAP)**;
   ch1 → 72h (pressure input for the loop, filtered into 42h);
   ch4 → 6Eh, ch5 → 6Fh, ch7 → 73h (temperature-type inputs with plausibility
   windows at `0x188A/0x188E`); ch0 → 69h, ch2 → 6Ah.
2. `64h = clamp(38h − 64) + f(RPM band)` — the load input for the big maps.
...
- ADC channel identities (ch1 vs ch3 both look like pressure; ch4 assumed IAT
  from the 551-series documentation) — confirm on the car with a scope or by
  logging the boost board inputs.
